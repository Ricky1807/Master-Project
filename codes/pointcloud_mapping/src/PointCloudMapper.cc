 ...
 */
    PointCloudMapper::PointCloudMapper() : nh("~"), spinner(0), it(nh) {
        // 相机内参
        float fx_, fy_, cx_, cy_, resolution_, depthfactor_,min_,max_;
        int queueSize_;
        bool mbuseExact_;
        ...
        nh.param<float>("mindistance", min_, 0.01);
        nh.param<float>("maxdistance", max_, 10.0);
        ...
        cout << "mindistance: " << mMin << endl;
        cout << "maxdistance:"  << mMax << endl;
...
//        time_t timer;
//        time(&timer);
//        string filename = to_string(timer) + "_single.pcd";
//        string save_path = "/home/ricky/Works/codes/result/" + filename;
//        pcl::io::savePCDFile(save_path, *tmp);

        cout << "========================mcx======================" << mcx << endl;
        cout <<  "size of tmp ==============: \n"<< tmp->points.size() <<  endl;
...
//        cloud_voxel_tem = tmp;
        ...
        cout <<  "size of cloud ==============: \n"<< cloud_voxel_tem->points.size() <<  endl;
 ...
 */
    void PointCloudMapper::viewer() {
        ...
        // 开启多线程监听
        ros::AsyncSpinner spinner(2); // Use 2threads
        spinner.start();
            ...
            if (KFUpdate ) {
                for (i = lastKeyframeSize; i < N && i < (lastKeyframeSize + 5); i++) {
                    if ((mvGlobalPointCloudsPose.size() != colorImgs.size()) ||
                        ...
                        cout << "====================" << endl;
                        cout << "mvGlobalPointCloudsPose.size() :" << mvGlobalPointCloudsPose.size() << endl;
...
//                    local.header.frame_id = "camera_link";
                ...
                }
                {
                    ...
                    if (i > (buff_length + 5)) {
                        ...
                        mvGlobalPointCloudsPose.erase(mvGlobalPointCloudsPose.begin(),
                                                      mvGlobalPointCloudsPose.end() - 5);

                        // ", depthImgs.beign(): " <<depthImgs.begin()<< ", colorImgs.begin(): "<<colorImgs.begin()<<
                        cout << "!!! depthImgs.size(): " << depthImgs.size() << endl;

                        depthImgs.erase(depthImgs.begin(), depthImgs.end() - 5);
                        colorImgs.erase(colorImgs.begin(), colorImgs.end() - 5);
                        i = i - buff_length;
...
// 复位点云显示模块
    ...
    void PointCloudMapper::shutdown() {
        ...
        time_t timer;
        time(&timer);
        string filename = to_string(timer) + ".pcd";
//        string save_path = "/home/ricky/result" + buf + ".pcd";
        string save_path = "/home/ricky/Works/codes/result/" + filename;
        cout << save_path;
